...
AS
	...
	-- //////////SE CREA TABLA TEMPORAR PARA GURAR LOS SQF TRANFERIDOS A LAS ORDENES/////////////////////////////////
	...
	AND LTRIM(RTRIM(ccjoblin_sql.jobno)) + RIGHT('000'+ LTRIM(RTRIM(ser_no)),3)  IN ( SELECT DISTINCT SERIAL_1 FROM INVENTARIO_EMBARQUE WHERE COLOR = @PP_COLOR AND K_ESTATUS_INVENTARIO_EMBARQUE IN (1,2))
	
	DECLARE @VP_TOTAL_SQF_FACTURADO DECIMAL(13,2) = 0
	SELECT  @VP_TOTAL_SQF_FACTURADO	=	SUM(CONVERT(DECIMAL(13,4),(OriginalQty * stdsqmper)))
	FROM  CCJOBLIN_SQL (NOLOCK) 
	WHERE JOBNO IN (SELECT DISTINCT ORDEN FROM @TBL_SQF_EN_ORDEN_ABIERTA)
	AND LTRIM(RTRIM(ccjoblin_sql.jobno)) + RIGHT('000'+ LTRIM(RTRIM(ser_no)),3)  IN ( SELECT DISTINCT SERIAL_1 FROM INVENTARIO_EMBARQUE WHERE COLOR = @PP_COLOR AND K_ESTATUS_INVENTARIO_EMBARQUE > 2)
	...
	DECLARE @VP_SQF_EN_PRD DECIMAL(13,2) = 0
	--DECLARE @VP_SQF_TRANSFERIDO_PRD DECIMAL(13,2) = @VP_TOTAL_SQF

	SET @VP_SQF_EN_PRD = @VP_SQF_TRANSFERIDO_PRD - @VP_TOTAL_SQF_MFP
